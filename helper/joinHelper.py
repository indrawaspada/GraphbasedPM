...
#     tuplePairs = entrance_exit_pairs
    finish = True
    if len(entrance_to_exit_pairs)>1:
        combinations = generalHelper.combinationPairInList(entrance_to_exit_pairs)
        for pair in combinations: # tiap entrance_exit di pasangkan
            j_len = len(joinNodeEnum[joinNode])
            if j_len>1:
                print('paaiiir= ', pair)
